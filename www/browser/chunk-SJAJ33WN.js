...
var CapacitorSQLite = registerPlugin("CapacitorSQLite", {
  web: () => import("./chunk-RITNSXAX.js").then((m) => new m.CapacitorSQLiteWeb()),
  electron: () => window.CapacitorCustomPlatform.plugins.CapacitorSQLite
});

// src/app/core/database/migrations/v1.migration.ts
var V1_MIGRATION = {
  version: 1,
  statements: [
    // ─── Materiales / Insumos ───────────────────────────────────────────────
...
// src/app/core/database/migrations/v2.migration.ts
var V2_MIGRATION = {
  version: 2,
  statements: [
...
// src/app/core/database/database.service.ts
var DB_NAME = "calisto_db";
var DB_VERSION = 2;
var DatabaseService = class _DatabaseService {
  ...
  ensureConnection() {
    return __async(this, null, function* () {
      const ret = yield this.sqlite.checkConnectionsConsistency();
      const isConn = (yield this.sqlite.isConnection(DB_NAME, false)).result;
      if (ret.result && isConn) {
        this.db = yield this.sqlite.retrieveConnection(DB_NAME, false);
      } else {
        this.db = yield this.sqlite.createConnection(DB_NAME, false, "no-encryption", DB_VERSION, false);
      }
      yield this.db.open();
    });
  }
  configurePragmas() {
    return __async(this, null, function* () {
      yield this.db.query("PRAGMA journal_mode = WAL;");
      yield this.db.query("PRAGMA foreign_keys = ON;");
      yield this.db.query("PRAGMA synchronous = NORMAL;");
      yield this.db.query("PRAGMA cache_size = 10000;");
    });
  }
  runMigrations() {
    return __async(this, null, function* () {
      const result = yield this.db.query("PRAGMA user_version;");
      ...
      if (currentVersion < V1_MIGRATION.version) {
        for (const stmt of V1_MIGRATION.statements) {
          yield this.db.execute(stmt);
        }
        yield this.db.execute(`PRAGMA user_version = ${V1_MIGRATION.version};`);
      }
      if (currentVersion < V2_MIGRATION.version) {
        for (const stmt of V2_MIGRATION.statements) {
          yield this.db.execute(stmt);
        }
        yield this.db.execute(`PRAGMA user_version = ${V2_MIGRATION.version};`);
      }
    });
  }
  ...
  getDb() {
    if (!this.db)
      throw new Error("DatabaseService no inicializado. Llama initialize() primero.");
    return this.db;
  }
  // ─── Helpers de query ─────────────────────────────────────────────────────
  query(_0) {
  ...
  transaction(fn) {
    return __async(this, null, function* () {
      yield this.db.execute("BEGIN TRANSACTION;");
      try {
        const result = yield fn();
        yield this.db.execute("COMMIT;");
        return result;
      } catch (error) {
        yield this.db.execute("ROLLBACK;");
        throw error;
      }
    });
  }
  static {
    this.\u0275fac = function DatabaseService_Factory(t) {
      return new (t || _DatabaseService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _DatabaseService, factory: _DatabaseService.\u0275fac, providedIn: "root" });
  }
};
...
//# sourceMappingURL=chunk-SJAJ33WN.js.map
