...
var e = (t, o) => {
  ...
  if (null != n2) {
    ...
    a2 = t.querySelectorAll("ion-buttons");
  for (const t2 of a2) {
    ...
    if (null !== s2 && ("start" === t2.slot || "" === t2.slot) && (e2 && a3 && o || !e2))
      return s2;
  }
  return null;
};
...
var l = (n2, l2) => {
  ...
  try {
    ...
    if (w.forEach((o) => {
      ...
      if (X2 && u2.addElement(X2), n3.addAnimation([s2, e2, f2, d2, u2]), e2.fromTo(p, 0.01, 1), f2.fromTo(p, 0.01, 1), v)
        l3 || s2.fromTo("transform", `translateX(${y})`, `translateX(${$})`).fromTo(p, 0.01, 1), f2.fromTo("transform", `translateX(${y})`, `translateX(${$})`), u2.fromTo(p, 0.01, 1);
      ...
      if (s2 || 0 !== e2.length || 0 !== r3.length ? (n3.addElement(s2), n3.addElement(r3)) : n3.addElement(x2.querySelector(":scope > .ion-page, :scope > ion-nav, :scope > ion-tabs")), k.addAnimation(n3), v) {
        ...
        n3.fromTo("transform", `translateX(${$})`, `translateX(${y})`).fromTo(p, 1, u);
      if (s2) {
      ...
      e2.forEach((o) => {
        ...
        if (x3 && X2.addElement(x3), n4.addAnimation([s3, e3, f2, X2, m2]), k.addAnimation(n4), X2.fromTo(p, 0.99, 0), e3.fromTo(p, 0.99, 0), f2.fromTo(p, 0.99, 0), v) {
          ...
          l3 || s3.fromTo("transform", `translateX(${$})`, `translateX(${y})`).fromTo(p, 0.99, 0).afterClearStyles([d, p]), f2.fromTo("transform", `translateX(${$})`, `translateX(${y})`).afterClearStyles([d, p]), X2.afterClearStyles([p]), s3.afterClearStyles([p]), e3.afterClearStyles([p]);
      });
    }
    return k;
  } catch (t) {
...
@ionic/core/components/p-CBzELu-H.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-V5M5BKA2.js.map
