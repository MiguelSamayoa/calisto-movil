...
var FilesystemWeb = class _FilesystemWeb extends WebPlugin {
  ...
  deleteFile(options) {
    return __async(this, null, function* () {
      ...
      if (entry === void 0)
        throw Error("File does not exist.");
      const entries = yield this.dbIndexRequest("by_folder", "getAllKeys", [
  ...
  rmdir(options) {
    return __async(this, null, function* () {
      ...
      const readDirResult = yield this.readdir({ path, directory });
      if (readDirResult.files.length !== 0 && !recursive)
        throw Error("Folder is not empty");
      for (const entry2 of readDirResult.files) {
        const entryPath = `${path}/${entry2.name}`;
        const entryObj = yield this.stat({ path: entryPath, directory });
        if (entryObj.type === "file") {
          yield this.deleteFile({ path: entryPath, directory });
        } else {
          yield this.rmdir({ path: entryPath, directory, recursive });
        }
      }
...
//# sourceMappingURL=chunk-4ELVC4KA.js.map
